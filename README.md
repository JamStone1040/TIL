...
- 모듈 활용
  - import 문 사용<br>'.'(dot 연산자)를 사용
  - from 절 사용<br> 필요한 것만 import로 가져오기
- 패키지
  
  연관된 모듈들을 하나의 디렉토리에 모아 놓은 것

  모듈들의 이름 공간을 구분하여 충돌을 방지하고, 모듈들을 효율적으로 관리할 수 있도록 돕는 역할

- 라이브러리
  
  라이브러리(도서관) > 패키지(책장 하나) > 모듈(책 한 권)

- 추가로 알면 좋은 것
  ```python
  from pprint import pprint
  ```
  복잡하게 출력되는 딕셔너리 등을 훨씬 더 가독성 좋게 출력해줌
...
- 조건문(if, elif, else)
  - if만 단독 사용 가능함(특정 상황에만 무언가를 덧붙일 때, 조건이 거짓이면 그냥 넘어감)
  - 조건의 순서는 중요한데, 파이썬은 조건을 순서대로 보기 때문임
  - 조건문 내부에 다른 조건문 작성 가능

- 반복문(for 반복, while 반복)
  - for 반복문
    - 반복 가능한 객체의 요소 개수 만큼 반복이 수행
    - 시퀀스 자료형(list, tuple, str, range)뿐만 아니라 비 시퀀스형 자료형(dict, set) 등도 반복 가능한 객체임
    - 반복 변수는 단수형으로 작성하는 것을 권장
    - 딕셔너리의 경우 반복 순서가 보장되지 않지만 작성된 순서는 보장(set은 아님)
    - 인덱스로 리스트를 순회하기 위해서 len()을 활용, 원하는 위치의 있는 값을 읽거나 변경할 수 있음 [i]
    - 중첩 반복문의 경우 본인 내부의 body(반복문)의 코드가 다 돌아야 자기 순서가 옴, 반복 개수는 길이들을 곱하면 됨, 이중 리스트를 개별 요소로 벗겨내기 위해서는 중첩 반복을 활용해야 함
  - while 반복문
    - 조건식이 false가 될 때 까지로 생각하면 조건을 만들기가 더 쉬워짐
    - 반드시 종료 조건이 필요함
  - 반복 제어
    
    
    for문과 while은 매 반복마다 본문 내 모든 코드를 실행하지만 때떄로 일부만 실행하는 것이 필요할 때가 있음
    - break<br>해당 조건의 키워드를 만나게 되면 남은 코드를 무시하고 즉시 종료
    - continue<br>해당 조건의 키워드를 만나게 되면 다음 코드는 무시하고 다음 반복을 수행(skip의 개념)
    - 들여쓰기 된 부분의 코드 블록의 반복 흐름만 제어함
    - 과도하게 남용하지 않기(가독성이 떨어짐)

[유용한 내장 함수 map]

- map(function, iterable)
  - 첫 번째 인자로 함수를 받고, 두 번째 인자로 반복가능한 객체를 받음
  - 요소 하나하나에 함수를 적용함
  - 그 결과 값들을 map object로 묶어서 반환, 실행(요리)은 아니고 아직 레시피 단계
  - 람다 표현식
    ```python
    def square(x):
      return x**2
  ...
  - 만약 함수의 인자가 여러개일 때 map함수 작성법은?
    ...
    # map에 리스트 2개를 순서대로 전달
    result = list(map(add, list1, list2))
    ```
  - map 함수 더 이해하기
    - 요리 시작 전까지 세팅
    - map을 하면 바로 결과를 출력하는게 아니라 준비를 맞춰놓음(계획서 == 이터레이터)
    - 파이썬에서 요리는 결국 형변환, 반복 등 임.
    - 반복자체가 요리기 때문에 for문을 위해 list()로 감쌀 필요 없음(oject 형태의 모든 것들도 마찬가지)
    - 근데 왜? : 항상 빅데이터 기준으로 생각하기, 메모리 절약을 위해서 
    - range(1)과 range(10억)은 같은 메모리, 둘다 각각 하나의 object로 관리하기 때문에

[참고]

- help
  - 모듈 자체에 대한 가이드를 보겠다
  - 엔터 누르면 -more- 더 볼 수 있음
  - 가장 좋은 방법은 공식문서에서 보는 것임
- for else
  - break를 안만나는 경우 else문으로 넘어감
- enumerate
  - (인덱스, 요소)를 튜플로 묶어서 뱉음
  - 그래서 반복변수에 언패킹이 됨
  - start를 1로 바꾸면 인덱스를 1부터 시작함
- zip함수
  - 묶어서 튜플로 묶어줌
  - 나중에 행렬때 사용(행과 열의 위치를 바꿀 때???)
  - zip객체가 나옴 → 지연평가
